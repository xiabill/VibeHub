...
左键状态栏图标 → 打开配置面板（宽 360），卡片式布局，两个 Tab。

### 按键录制（两个 Tab 通用）

每条绑定行显示成：**启用开关 + 手势名 + chord 键帽 chips**（如 `⌥`、`F13`、`A`）+ **录制按钮** +（AirPods）模式菜单 + **⋯ 手动编辑菜单**。

- **录制**：点行尾录制按钮 → chips 区变成高亮的「按下快捷键…」→ 直接按下目标组合键即可绑定。
  - 普通键：按住修饰键 + 主键（如 `⌘ + C`），松手即录入。
  - 纯修饰键 chord（如只绑 Left Option，Typeless 刚需）：按下再全部松开即录入，可区分左右修饰键。
  - `Esc`（无修饰）取消录制；键盘上没有的键会「哔」一声并保持录制态。
  - 录制期间只捕获本 App，吞掉按键不触发系统快捷键；同一时刻只有一行在录制，切 Tab / 关面板自动取消。
- **⋯ 手动编辑**：逐键改 / 删键 / 追加键。F13–F20 这类键盘上按不出来的键靠它选。

### AirPods Tab

- **模块卡**：耳机图标 + 名称 + `ⓘ`（机型支持说明）+ `⋯` 菜单（重置默认 / 从旧版导入）+ **启停开关**（无权限时自动弹回）；下方状态圆点（运行中 / 已暂停）。
- **权限卡**：缺「辅助功能」时橙色卡引导，授权后实时变 ✓ 并提示重启；权限齐全时不显示。
- **绑定卡**：单击 / 双击 / 三击 / 音量+ / 音量-，卡头 `ⓘ` 解释「点按 / 按住」。绑定音量键时提示耳机将无法调节系统音量。

### Remote Tab

- **模块卡**：遥控图标 + 名称 + `⋯` 菜单 + **启停开关**；下方状态圆点（运行中 / 等待设备 / 已暂停）+ 当前设备 hex id；出错时一行橙字 + 重启按钮。
- **权限卡**：检查「辅助功能」+「输入监听」（只查不弹框），授权后提示重启。
- **绑定卡**：12 颗按键分 3 组（方向 / 确认、系统功能、音量）。
- **设备卡**：目标设备 hex + 「切换…」（下拉选已插上的 HID 键盘类设备 / 重新扫描 / 恢复默认）+ **最近按键**回显（已知按键显示名称与 usage；未知按键橙字提示「可通过学习新按键添加」，配合自学习发现遥控器上未收录的键）+ 自定义按键（自学习：点「学习新按键」→ 10 秒内按遥控器上任意未收录的键 → 命名保存；若按到已收录的键会橙字提示「该按键已存在」并继续等待）。
- **高级卡**：长按自动重复开关 + 启动延迟（200–1500ms）+ 重复间隔（30–500ms）滑块。
- **配置模式**：面板打开期间按遥控器按键只回显与高亮、**不执行绑定**（避免切窗 / Enter 抢焦点把面板顶掉、打断命名输入）；关闭面板后恢复正常执行。

### 全局

- 面板底部：开机自启 toggle + 版本号 + 退出。
- 右键状态栏图标 → 快捷菜单：分别启停两个模块、当前绑定快览（含自定义按键）、权限设置、开机自启、关于、退出。
- 状态栏图标为程序内绘制的波形 template 图标；AirPods 处于按住态变红，Remote 等待设备变橙。
...
`VibeHub.swift`（单文件，~2310 行 Swift / SwiftUI / Cocoa）
