...
- **设备卡**：目标设备 hex + 「切换…」（下拉选已插上的 HID 键盘类设备 / 重新扫描 / 恢复默认）+ 自定义按键（自学习：点「学习新按键」→ 10 秒内按遥控器上任意未收录的键 → 命名保存）。
...
`VibeHub.swift`（单文件，~2210 行 Swift / SwiftUI / Cocoa）
